...
'''
python3 -m service.test.gnmi_subscribe --submode 1 --encoding 2 "/port_metrics[name=P1]" "/flow_metrics[name=F1]" "/bgpv4_metrics[name=BGP Peer 1]" "/flow_metrics[name=F2]"
python3 -m service.test.gnmi_subscribe --server 10.72.47.36:50051 --submode 1 --encoding 2 "/port_metrics[name=P1]" "/flow_metrics[name=F1]" "/bgpv4_metrics[name=BGP Router 1]" "/flow_metrics[name=F2]"
			...
			for response in responses:
				...
				if response.HasField('sync_response'):
					...
					if self.options.stats:
						self.logger.info("%d updates and %d messages within %1.2f seconds", upds, msgs, secs)
						self.logger.info("Statistics: %5.0f upd/sec, %5.0f msg/sec", upds/secs, msgs/secs)
				elif response.HasField('error'):
					self.logger.error('gNMI Error '+str(response.error.code)+' received\n'+str(response.error.message))
