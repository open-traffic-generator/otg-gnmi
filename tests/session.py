...
from otg_gnmi.common.utils import init_logging, generate_subscription_request
import json
...
class Session(object):
    ...
    def is_secure(self):
        if self.options.secure and len(self.options.cert) == 0:
            return False
        return True
            ...
            for response in responses:
                ...
                if response.HasField('error'):
                    ...
                    if self.options.stats:
                        self.logger.info("Total Messages: %d [Rate: %5.0f], Total Updates: %d [Rate: %5.0f], Total Time: %1.2f secs", # noqa
                                         resp_cnt,
                                         resp_cnt / secs,
                                         upd_cnt,
                                         upd_cnt / secs,
                    ...
                    self.logger.error('Received unknown response: %s', str(
                        response))
