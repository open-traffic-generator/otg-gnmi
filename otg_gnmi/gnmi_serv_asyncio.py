...
class AsyncGnmiService(gnmi_pb2_grpc.gNMIServicer):
    ...
    async def Capabilities(self, request, context):
        ...
        the target supports.
        The models can then be specified in subsequent RPCs
            ...
            try:
                ...
                for response in responses:
                    if response is not None:
                        ...
                        yield response
                counter = counter + 1
                if (session.mode == gnmi_pb2.SubscriptionList.Mode.ONCE or session.mode == gnmi_pb2.SubscriptionList.Mode.POLL) and session.sent_sync is True: # noqa
                    self.logger.info(
