...
from .utils import RequestType, get_subscription_type, gnmi_path_to_string
...
class TestManager:
        ...
        if context in self.client_sessions:
            ...
            try:
                await asyncio.wait_for(
                    self.parse_requests(
                        request_iterator, requests), timeout=1.0)
            except asyncio.TimeoutError as ex:
                self.logger.error(
                    'Parse request timed out exception: %s', str(
                        ex))
    ...
    def dump_all_subscription(self):
        ...
        self.logger.info(
            'Protocol Subscriptions: total subscription = %s', len(
                self.protocol_subscriptions))
    ...
    async def register_subscription(self, session):
        self.lock.acquire()
        self.logger.info(
            'Register Subscription for %s elements', len(
                session.requests))
        try:
    ...
    async def deregister_subscription(self, session):
        ...
        self.logger.info('Deregister Subscription for %s elements', len(
            session.requests))
        try:
            for request in session.requests:
                if request is None:
