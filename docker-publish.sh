...
publish() {
    ...
    echo "Main Branch ..."
    DOCKERHUB_IMAGE="${DOCKERHUB_REPO}/ixia-c-gnmi-server:${TAG}"
    dockerhub_image_exists "${DOCKERHUB_IMAGE}"
    echo "Publishing image to DockerHub..."
    docker tag ixia-c-gnmi-server "${DOCKERHUB_IMAGE}"
    ...
    docker rmi -f "ixia-c-gnmi-server" "${DOCKERHUB_IMAGE}"> /dev/null 2>&1 || true
